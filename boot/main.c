...
#ifdef CONFIG_UPV_SERIAl
extern int uart_send_buffer(U32 port_id, void *buff, U32 len);
#endif

static char goUpgradeLoop[16] = "EnterUpgrade";
...
U32 auto_upgrade(void)
{
  ...
  #endif
                ret = 1;
                uart_send_buffer(port,goUpgradeLoop,16);
                printf("response Enter Upgrade Mode.port = %d\n",port);
                return ret;
            }
        }
        e = arch_counter_get_current();
...
AUTO_UPGRADE:
    ...
    while (1){
        ...
        if(count%100 == 0)
        {
            printf("No upgrade tool detected. detected interval = %dms, interval count = %d.Continue.\n",
                    CONFIG_AUTO_UPGRADE_DELAY_MS,count);
            count = 0;
        }
#endif
        ;
