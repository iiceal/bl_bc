...
static inline u32 get_part_sector_size_nor(struct mtdids *id, struct part_info *part)
{
    ...
    printf("%s: start[0] = 0x%x, num = %d \n", __func__, flash->start[0], id->num); 
	start_phys = (u32)(flash->start[0] + part->offset);
	end_phys = (u32)(start_phys + part->size - 1);
    printf("%s: part->offset 0x%x part->size 0x%x start_phys 0x%x end_phys 0x%x\n",
            __func__, part->offset, part->size, start_phys, end_phys);
    printf("flash->sector_count %d \n", flash->sector_count);
    ...
    sector_size = part->size/flash->sector_count;
    printf("%s: part->offset 0x%x sector_size 0x%x\n", __func__, part->offset, sector_size);
	return sector_size;
#else
	BUG();
	return 0;
...
int mtdparts_init(void)
{
	...
	if (!initialized) {
        ...
        printf("current_mtd_dev 0x%x size %d id 0x%x size %d part 0x%x size %d \n",
                current_mtd_dev, sizeof(struct mtd_device), id, sizeof(struct mtdids),
                part, sizeof(struct part_info));
		/* id */
		...
		id->size = CONFIG_JFFS2_PART_SIZE; //size;
		INIT_LIST_HEAD(&id->link);

		printf("dev id: type = %d, num = %d, size = 0x%x, mtd_id = %s, ssss=%d\n",
				id->type, id->num, id->size, id->mtd_id, CONFIG_JFFS2_PART_SIZE);
		...
		part->size = 0x400000; //CONFIG_JFFS2_PART_SIZE;
        printf("%s:part->size 0x%x\n", __func__, part->size);
#else
		part->size = SIZE_REMAINING;
        printf("%s:-iii---part->offset 0x%x part->size 0x%x\n", __func__, part->offset, part->size);
#endif
        ...
        printf("%s:part->offset 0x%x part->size 0x%x\n", __func__, part->offset, part->size);
		...
		printf("part  : name = %s, size = 0x%x, offset = 0x%x sector_size 0x%x\n",
				part->name, part->size, part->offset, part->sector_size);
 ...
 * @return 0 on success, 1 otherwise
 */
int do_jffs2_fsload(char *filename, u32 offset)
{
	//char *fsname;
	int size;
	struct part_info *part;
	...
	if ((part = jffs2_part_info(current_mtd_dev, current_mtd_partnum))){
		...
		if (size > 0) {
			printf("### load complete: %d bytes loaded to 0x%x\n", size, offset);
...
static BL_CMD_T jffs2_descriptor;

static u32 jffs2_cmd_entry_func(BL_CNXT_T *pCnxt, u32 argc, const char **argv)
{
	char *filename = "/";
	u32 offset=0x20000000;
	...
	if ((argc >= 2) && (strcmp(argv[1], "fsinfo") == 0)) {
		do_jffs2_fsinfo();
	}

	if ((argc >= 2) && (strcmp(argv[1], "fsls") == 0)) {

		if (argc == 3)
			filename = (char *)argv[2];
		do_jffs2_ls(filename);
	}

	if ((argc >= 3) && (strcmp(argv[1], "fsload") == 0)) {
		filename =(char *)argv[2];

		if(argc == 4)
			offset = simple_strtoul(argv[3], NULL, 10);
		do_jffs2_fsload(filename, offset);
	}
...
{
    return (u8 *) "Case list: jffs2 fsload/fsinfo/fsls etc.\n";
}

static u8 *long_description(void)
{
    return (u8 *) "Description: The applicaition running command is as follow:\n\
               	jffs2 fsload :  load binary file from a filesystem image \n\
                jffs2 fsls : list files in a directory (default /) \n\
                jffs2 fsinfo : print information about filesystems \n";
}

static BL_CMD_T *static_cmd_descriptor(BL_CNXT_T *pCnxt)
{
    BL_CMD_T *p_descriptor = &jffs2_descriptor;
    memset(p_descriptor, 0, sizeof(BL_CMD_T));

    memcpy(&p_descriptor->name[0], "jffs2", BL_CMD_SHORT_NAME_LENGTH);

    p_descriptor->cmd_entry_func = jffs2_cmd_entry_func;
