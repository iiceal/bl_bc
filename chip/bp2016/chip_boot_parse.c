...
bool check_bl2fw_hdr(char * bl_name, u32 bl2fw_type, u32 * fl_addr)
{
    ...
    // first block
    flash_dma_read_start(*fl_addr,rd_hdr_buf, fl_pg_size);
    flash_dma_read_wait_done();
    flash_dma_read_start(*fl_addr + fl_pg_size,rd_hdr_buf + fl_pg_size, fl_pg_size);
    flash_dma_read_wait_done();
